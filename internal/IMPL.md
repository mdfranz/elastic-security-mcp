...
- **Response truncation**: three near-duplicate "shrink an array proportionally to fit under `MaxResponseChars()`" implementations exist — `truncateResults`/`truncateSlice` in `tools.go` (raw ES response shape, also strips `_index`/`_id`/`_score` per hit; `truncateSlice` sizes the array against the *whole* budget with no margin — correct only when the array essentially *is* the whole payload, e.g. `list_indices`), `truncateSecuritySearchResults` in `security_search.go`, and `truncateSecurityStatsBuckets` in `security_stats.go` (the latter two both marshal the *whole enveloping result* up front and apply a 10% margin before truncating, since their array sits inside a larger structure plus metadata the function itself adds afterward — sizing the bare array alone against the full budget would leave no room for that overhead and made an earlier version of `truncateSecurityStatsBuckets` spuriously hit its own "cannot fit" error on every large response). Touching truncation behavior means updating all three.
...
### security_stats.go
Implements `search_security_stats`: one bounded terms/date_histogram/cardinality aggregation over security event data, `size:0` so no raw documents are returned.

- `SecurityStatsArgs{Index, Query, Field, AggregationType, Interval, Start (required), End (required), Size, IncludeTotal, PrecisionThreshold}`. Unlike the other search tools, `Start`/`End` are **required**, not optional — an explicit window is what makes the bucket-count pre-flight check possible.
- Config (env-overridable, `init()`-set like `tools.go`): `StatsMaxRangeHours()` (744h/31 days, `STATS_MAX_RANGE_HOURS`), `StatsMaxBuckets()` (250, `STATS_MAX_BUCKETS`).
- `normalizeSecurityStatsArgs`: trims/lowercases `aggregation_type`; parses `Start`/`End` as RFC3339 (`time.Parse`, unlike every other tool in this package which passes start/end straight through to Elasticsearch as strings — needed here to compute the range/bucket checks in Go before querying); rejects a reversed or over-`StatsMaxRangeHours` range; per-type validation (terms/cardinality require `Field`; date_histogram defaults `Field` to `@timestamp` and `Interval` to `1h`). Size defaults 10/caps 100 (terms only); `PrecisionThreshold` validated 1–40000 if set (cardinality only).
- Calendar vs. fixed interval classification is case-sensitive and exact-match against `calendarShorthandIntervals` (`1m` minute, `1h`, `1d`, `1w`, `1M` month, `1q`, `1y` — `1m`/`1M` must not collapse). Anything else must match `fixedIntervalPattern` (`^[1-9][0-9]*(ms|s|m|h|d)$`, e.g. `15m`, `6h`). **Gotcha**: `typedapi/types/enums/calendarinterval.CalendarInterval` only exposes named-word constants (`Hour`, `Month`, ...); the request builder does *not* use them — it constructs `calendarinterval.CalendarInterval{Name: args.Interval}` directly with the raw shorthand, since `MarshalText` emits `Name` verbatim and Elasticsearch's `calendar_interval` parser accepts the shorthand form on the wire.
- `estimateBucketCount(start, end, interval, isCalendar, cap)`: for fixed intervals, exact `range/duration`. For calendar intervals, steps `start` forward one interval at a time (`addCalendarInterval`, using `time.AddDate` for month/quarter/year so variable month length is handled exactly rather than approximated) and bails out as soon as the count exceeds `cap` — the caller only needs "exceeds the limit", not a precise count for oversized ranges.
- `buildSecurityStatsRequest`: `Size: 0`; reuses `buildTimestampFilter` from `security_search.go` for the required `@timestamp` range and adds an optional `query_string` filter from `Query`; exactly one named `stats` aggregation (`terms`/`date_histogram`/`cardinality`); `TrackTotalHits` set from `IncludeTotal` (default `false`, unlike `search_security_events`'s hardcoded `true`).
- `runSecurityStatsSearch`: calls `.TypedKeys(true)` explicitly on the search request even though Elasticsearch's `_search` endpoint already defaults `typed_keys` to `true` server-side — belt-and-suspenders so decoding doesn't silently break if that default ever changes or a proxy strips the parameter. Rejects `resp.TimedOut` or `resp.Shards_.Failed > 0` as errors rather than returning partial telemetry as if it were complete. Logs the built request JSON at DEBUG (`slog.Debug("search_security_stats query", ...)`), matching `search_elastic`/`search_security_events` — added after a live-cluster log review found a 20-100x latency outlier on one `terms` query with no way to inspect what request had actually run.
- `shapeSecurityStatsResponse` type-switches `resp.Aggregations["stats"]` (relies on the `sterms#stats`/`date_histogram#stats`/`cardinality#stats` wire prefix for the typed client to pick the right `Aggregate` variant): `shapeTermsAggregate` handles `*types.StringTermsAggregate`/`*types.LongTermsAggregate`/`*types.DoubleTermsAggregate`/`*types.UnmappedTermsAggregate` (the last is a valid empty result, not an error — the field just isn't mapped in the selected indices, and is flagged with `"unmapped": true` plus an explanatory `"note"` so it isn't indistinguishable from a mapped field with zero matches — a live session was observed guessing a wrong field name and burning extra round trips because both cases rendered as an identical empty `buckets: []`); `shapeDateHistogramAggregate`; `shapeCardinalityAggregate` (always marks `approximate: true`).
- `formatStatsTotalEvents` is a **separate function from `formatTotalHits`** (`security_search.go`), not a reuse — `formatTotalHits`'s nil-total case reports `{value:0, relation:"eq"}`, which is safe there because those callers always set `TrackTotalHits: true`. Here `IncludeTotal` defaults to `false`, so `resp.Hits.Total` is `nil` on nearly every call; reusing `formatTotalHits` would misreport "we didn't ask for a count" as "we found exactly zero documents". Instead reports `{value: nil, relation: "unknown", exact: false}` when total is nil.
- `truncateSecurityStatsBuckets` (terms only): mirrors `truncateSecuritySearchResults` (marshal the whole envelope, proportional shrink with a 10% margin) rather than the generic `truncateSlice` — see cross-cutting truncation note for why. If the payload still doesn't fit `MaxResponseChars()` after truncating to the smallest possible bucket list, returns an error instead of emitting an ambiguous partial result.
- `RegisterSecurityStatsTool`: wrapped with `WrapWithCache(cache, "search_security_stats", SearchSecurityStatsTTL(), ...)`; args normalized before entering the cache wrapper (consistent cache keys); registered in `RegisterTools` (`tools.go`) before `search_security_events`.
...
Tested by `security_stats_test.go`: normalization (whitespace/defaults/caps, every rejection path — blank index, unsupported aggregation, missing field, invalid/reversed/oversized timestamps, invalid intervals, invalid precision threshold, bucket-limit rejection), calendar-vs-fixed interval classification (`1M` preserved as month, not lowercased), request JSON per aggregation type (`size:0`, filters, terms size, cardinality precision threshold, default-false vs. opt-in `track_total_hits`), response shaping for terms/date-histogram/cardinality/unmapped-terms, terms truncation and the "cannot fit" error path, and timed-out/failed-shard rejection. A fake-`http.RoundTripper` transport test (reusing the pattern from `security_search_test.go`) exercises `runSecurityStatsSearch` end-to-end against `sterms#stats`/`date_histogram#stats`/`cardinality#stats` wire responses.
